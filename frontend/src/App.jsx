...
import {AlertCircle, CheckCircle, Clock, Download, FileText, RefreshCw, Send, Upload} from 'lucide-react';
...
const App = () => {
    ...
    useEffect(() => {
        const interval = setInterval(async () => {
            const response = await loadProcessedFiles(); // this must return the reconciliations data

            const isIncomplete = response?.data?.reconciliations?.some(
                (r) => r.status !== "completed"
            );

            if (isIncomplete) {
                console.log("Reconciliation still pending...");
                // optionally update state or show loading UI
            } else {
                console.log("All reconciliations completed, stopping refresh");
        ...
        <div className="flex h-screen bg-gray-50">
            ...
            <div className="flex-1 flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    ...
                    {isProcessing && (
                        <div
                            className="flex items-center space-x-3 text-blue-600 bg-blue-50 p-4 rounded-lg mr-auto max-w-md">
                    ...
                    {isAnalyzingColumns && (
                        <div
                            className="flex items-center space-x-3 text-purple-600 bg-purple-50 p-4 rounded-lg mr-auto max-w-md">
                            ...
                            {currentInput ? (
                                ...
                                <div className="p-3 border border-gray-200 rounded-lg bg-yellow-50 border-yellow-200">
                                    <div className="text-sm text-yellow-800">
                                        👈 Please select a template from the left panel to load reconciliation
                                        requirements.
                                    </div>
                                </div>
                            )}
                    ...
                    {processedFiles.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {processedFiles.map((reconciliation) => (
                                <div key={reconciliation.reconciliation_id}
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center space-x-2">
                                            ...
                                            <span
                                                className="text-sm font-medium text-gray-800 capitalize">{reconciliation.status}</span>
                                        </div>
                                    </div>

                                    <div className="text-xs text-gray-600 mb-2">
                                        <div className="truncate" title={reconciliation.file_a}>📄
                                            A: {reconciliation.file_a}</div>
                                        <div className="truncate" title={reconciliation.file_b}>📄
                                            B: {reconciliation.file_b}</div>
                                    </div>
                                    ...
                                    {reconciliation.status === 'completed' && (
                                        <>
                                            <div className="text-xs text-gray-600 mb-3 bg-gray-50 p-2 rounded">
                                                ...
                                                <div>🎯
                                                    Confidence: {(reconciliation.match_confidence || 0).toFixed(1)}%
                                                </div>
                                            </div>
